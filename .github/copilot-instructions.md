...
- **GM View** (`/gm`) - Full map control, token management, fog of war configuration
- **Observer View** (`/observer`) - Read-only map display for spectators/projection
- **Player View** (`/player`) - Individual character card + character sheet iframe

**Why Separate?** Initially had one "player" route for map viewing, but users need different experiences:
- Observers passively watch the map (no interaction)
- Players need character cards, stat tracking, and heartbeat system
...
- Observer View: http://localhost:5173/observer
